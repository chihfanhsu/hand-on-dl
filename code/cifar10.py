...
for fn in train_fns:
	...
	if fn == train_fns[0]:
		...
		img_px_values = #利用 vstack/hstack/dstack 將 img_px_values 與 raw_data['data']接起來
		img_lab = #利用 vstack/hstack/dstack 將 img_lab 與 raw_data['labels'] 接起來

# convert 1d-ndarray (0:3072) to 3d-ndarray(32,32,3)
X_train = #請用 reshape 將 3072 維度的 list 轉換成 32x32 的 RGB 影像
          #再利用 vstack/hstack/dstack 將三張影像接起來
          #可以嘗試 [ ... for r in img_px_values] 的格式處理

Y_train = # 請將 labels 轉成 one-hot encoding 的形式
